...
class CreateBundle(object):
    ...
    def __init__(self, app_name, version, start_up_file, bundle_identifier='luxshare-ict'):
        self._app_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/' + app_name + '.app'
    ...
    def make_script_executable(self):
        ...
        os.chmod(self._app_path + "/Contents/MacOS/{}".format('main'),
                 33277 | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)

    def cp_all_folders(self):
        os.makedirs("{0}/Contents/Resources/".format(self._app_path))
        cmd = 'cp ./dist/{} "{}/Contents/Resources"'.format(self._bundle_name,self._app_path)
        os.system(cmd)
        os.makedirs("{0}/Contents/Resources/Config".format(self._app_path))
        cmd = r'cp -a ./Config/. "{}/Contents/Resources/Config"'.format(self._app_path)
        os.system(cmd)

        cmd = r'cp ./AppIcon.icns "{}/Contents/Resources/"'.format(self._app_path)
        os.system(cmd)

    def mv_resources_folder(self):

        # cmd = r'mv {0}/Contents/Resources/AppIcon.icns "{0}/Contents/Resources/"'.format(
        #     self._app_path)
        # os.system(cmd)
        cmd = r'rm -rf {}/Contents/Resources/{}'.format(self._app_path, __file__)
        os.system(cmd)

    def delete_all_py(self):
        code_path = "{}/Contents/Resources/.code/GUI".format(self._app_path)
        self.delete_py_in_path(code_path)
        # os.remove("{}/Contents/Resources/{}".format(self._app_path, __file__.split('/')[-1]))
        # sub folders of code
    ...
    def create_app_script(self):
        # new_python = os.path.join(self._app_path, "Contents", "MacOS", 'main')
        # python_path = os.path.join(self._app_path, "Contents", "Resources", 'main')
        # # print(python_path)
...
if __name__ == '__main__':
    app_name = 'H28_SIP_FWDL'
    cmd = 'cd {} & Pyinstaller -F -w {}/main.py -n{}'.format(os.path.dirname(os.path.abspath(__file__)),os.path.dirname(os.path.abspath(__file__)),app_name)
    os.system(cmd)
    bundle = CreateBundle(app_name, '0.0.01', 'app_name')
